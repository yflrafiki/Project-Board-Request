...
export const MarketingTeam = () => {
  ...
  const statusCounts = STATUS_ORDER.map((status) => ({
    status,
    count: marketingTasks.filter((t) => t.status === status).length,
  }));

  const filteredTasks = activeStatus
    ? marketingTasks.filter((t) => t.status === activeStatus)
    : marketingTasks;
  ...
  return (
    <div className="min-h-screen px-4 py-6 sm:px-6 xl:px-12 w-full max-w-screen-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">Overview of all marketing-related project tasks</p>
      </div>
